...
public class Player : MonoBehaviour {
	...
	void Update () {
		if (Input.GetKey("w") && player_1.transform.position.y < 7.5f)
				player_1.transform.Translate(Vector3.up * speed * Time.deltaTime);
		if (Input.GetKey("s") && player_1.transform.position.y > -7.5f)
				player_1.transform.Translate(Vector3.down * speed * Time.deltaTime);				
		if (Input.GetKey("up") && player_2.transform.position.y < 7.5f)
				player_2.transform.Translate(Vector3.up * speed * Time.deltaTime);
		if (Input.GetKey("down") && player_2.transform.position.y > -7.5f)

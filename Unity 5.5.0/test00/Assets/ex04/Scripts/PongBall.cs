...
public class PongBall : MonoBehaviour {
	...
	void Update () {
		...
		//Bounces on the wall
		if (transform.position.y >= 9.2 || transform.position.y <= -9.2)
			direction = new Vector3(direction.x, -direction.y, direction.z);
		else if (checkIncrement && (transform.position.x >= 5.2 && (player_2.transform.position.y + 2f >= transform.position.y && player_2.transform.position.y - 2 <= transform.position.y)) || checkIncrement && (transform.position.x <= -5.2 && (player_1.transform.position.y + 2f >= transform.position.y && player_1.transform.position.y - 2 <= transform.position.y))) {
		...
		else if (transform.position.x < 5.2 && transform.position.x > -5.2)
			checkIncrement = true;
	}
}

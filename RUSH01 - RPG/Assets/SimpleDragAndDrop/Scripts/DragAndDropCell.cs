...
public class DragAndDropCell : MonoBehaviour, IDropHandler
{
    ...
    public bool delete_one = false;
    public GameObject deleted_object;
        ...
        if (myItem != null)
        {
            if (myItem == item)
            {
                ...
                if (rayresult.Count > 0) {
                    ...
                    if (rayresult[0].gameObject.tag != "Inventory" && rayresult[0].gameObject.tag != "Slot") {
                        delete_one = true;
                        deleted_object = myItem.gameObject;

                        Debug.Log(myItem.gameObject);
    ...
    public void OnDrop(PointerEventData data)
    {
        ...
        else if (DragAndDropItem.icon == null) {
